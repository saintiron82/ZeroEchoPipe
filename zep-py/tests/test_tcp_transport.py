"""ZEP TCP Transport tests — Frame Profile over TCP localhost."""
...
from zep.transport.tcp import TcpTransport, frame_encode, frame_decode
...
class TestSocketRoundtrip(unittest.TestCase):
    """Two peers communicate via TcpTransport over TCP localhost."""

    def test_basic_call(self):
        server = TcpTransport(0, is_server=True)
        port = server.port
        time.sleep(0.05)  # let server start

        client = TcpTransport(port, is_server=False)
